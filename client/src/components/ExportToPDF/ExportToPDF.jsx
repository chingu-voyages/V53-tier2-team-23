...
const ExportToPDF = ({ weekDates, getImageURL }) => {
  const exportToPDF = () => {
    ...
    weekDates.forEach((item, index) => {
      ...
      const imageURL = getImageURL(item.dish ? item.dish.imageUrl : null);

      console.log(`Dish exists: ${dishExists}`); // Debugging
          ...
          dishExists
            ...
            <div class="border-[1px] border-primary rounded-3xl w-full mt-[0.9rem] p-2 text-center pb-4">
              ...
              <div class="mt-2 mx-1">
                ...
                item.dish.dishName
              }" onError="this.src='https://placehold.co/400'" class="rounded-xl border-[3px] border-primary object-cover h-full w-full" />
              </div>
